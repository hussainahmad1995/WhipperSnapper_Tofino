...
def add_layers(nb_fields, nb_headers):
    class header(Packet):
        ...
        for i in range(nb_fields):
            fields_desc.append(XBitField(name = 'field_%d' % i ,default = None, size = 16 , tot_size= 16))
    layers = ''
...
if __name__ == "__main__":
    ...
    pkt/= add_layers(nb_fields = 1 , nb_headers = args.nbheaders)
    ...
    reply = srp(pkt , iface = iface)
    end_time = time.time()

    latency = (end_time - start_time ) * time 
